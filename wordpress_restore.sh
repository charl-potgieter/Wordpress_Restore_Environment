...
echo "WHAT PASSWORD NEEDS TO BE ENTERED HERE AND WHY?"
printf "\n"
sleep 1

mysql -u root -p wordpress   <   $targetpath/database/mysql.database
mysql -p wordpress   <   $targetpath/database/mysql.database
...
sed -i -e "s/www.example.com/${domain}/g" $sql_filename

#Execute above sql to update from live url domain to localhost
